...
public class Help {
	...
	public static void printHelp(final OptionHelpWriter w) {
		...
		try {
			...
			throw new RuntimeException(e); // sshould never happen
		}
		...
		w.writeOptionHelp("speedup", "speedup", HELP_SPEEDUP + "; complementary to --total-time option", false, cfg.getSpeedup());
		w.writeOptionHelp("total-time", "time", "total length of video in milliseconds; complementary to --speedup option", false, cfg.getTotalTime());
		w.writeOptionHelp("fps", "fps", "frames per second", false, cfg.getFps());
		w.writeOptionHelp("width", "width", "video width in pixels; if not specified but --zoom option is specified, then computed from GPX bounding box and margin, otherwise 800", false, "(800)"); // cfg.getWidth()
		w.writeOptionHelp("height", "height", "video height in pixels; if unspecified, it is derived from width, GPX bounding box and margin", false, cfg.getHeight());
		...
		w.writeOptionHelp("tms-url-template", "template", "slippymap (TMS) URL template for background map where {x}, {y} and {zoom} placeholders will be replaced; " +
				"for example use http://tile.openstreetmap.org/{zoom}/{x}/{y}.png for OpenStreetMap", false, cfg.getTmsUrlTemplate());
		w.writeOptionHelp("background-map-visibility", "visibility", "visibility of the background map from 0.0 to 1.0", false, cfg.getBackgroundMapVisibility());
		w.writeOptionHelp("font-size", "size", "datetime text font size; set to 0 for no date text", false, cfg.getFontSize());
		w.writeOptionHelp("keep-idle", null, "don't skip parts where no movement is present", false, !cfg.isSkipIdle());
		w.writeOptionHelp("flashback-color", "ARGBcolor", "color of the idle-skipping flashback effect in #AARRGGBB representation", false, "opaque white - #ffffffff"); // TODO cfg.getFlashbackColor()
		w.writeOptionHelp("flashback-duration", "duration", "idle-skipping flashback effect duration in milliseconds; set to \"\" for no flashback", false, cfg.getFlashbackDuration());
